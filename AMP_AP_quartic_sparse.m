...
%a look at ZF_AMPfull_quartic.m
...
epsl = sqrt(0.7); % correlation of initialization
    ...
    for i = 1 : ntrials
        ...
        for jj = 2 : niter-1
           ...
           if mod(jj,3) ~= 0
               ...
               uAMP(:, jj+1) = sqrt(rho).*tanh(muSE(1:jj)'/Sigmared*(fAMP(:, 1:jj)')./sqrt(rho))'./...
                   (rho+(1-rho).*sech(muSE(1:jj)'/Sigmared* fAMP(:, 1:jj)'./sqrt(rho))'.*exp(muSE(1:jj)'/Sigmared*muSE(1:jj)/2/rho)); 
           ...
           if mod(jj,3) ~= 0
               ...
               Phi(jj+1, 1:jj) = mean( 1./(rho+(1-rho).*exp(muSE(1:jj)'/ Sigmared *muSE(1:jj)./2./rho).*sech(muSE(1:jj)'/Sigmared*(fAMP(:, 1:jj)')./sqrt(rho))') ...
                   - rho.* tanh(muSE(1:jj)'/Sigmared*(fAMP(:, 1:jj)')./sqrt(rho))'.^2 ./...
                   (rho+(1-rho).*exp(muSE(1:jj)'/ Sigmared *muSE(1:jj)./2./rho).*sech(muSE(1:jj)'/Sigmared*fAMP(:, 1:jj)'./sqrt(rho))').^2)...
                   * muSE(1:jj)' / Sigmared;
...
save ZFmod_sparse_n12k_eps0dot7_u0_SNR3dot8.mat alphagrid scal_all MSE;
