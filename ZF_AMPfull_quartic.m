...
%rho=0.3; if we want a sparse signal
...
for j = 1 : length(alphagrid)
    ...
    for i = 1 : ntrials
        ...
        for jj = 2 : niter-1
           ...
           uAMP(:, jj+1) = tanh(muSE(1:jj)'/Sigmared*(fAMP(:, 1:jj)'))'; % HERE IS THE DIFFERENCE: the denoiser
           %uses all the past iterates to evaluate the argument of tanh.
           ...
           % The past iterates are used also to update \Phi.
...
save ZFAMP_n4k_eps0dot5_u0 alphagrid scal_all MSE;
